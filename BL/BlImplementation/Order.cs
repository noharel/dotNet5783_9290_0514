...
internal class Order : BlApi.IOrder
{
        ...
        if ((orderId >= 0)&& (orderId.ToString().Length == 4))  // VALID ORDER ID
        {
            try
            {
                ...
                try
                {
                    ...
                    var BOlistOrder = from item in listOrder
                                      select new BO.OrderItem() { ID = (int)item?.ID!, Amount = (int)item?.Amount!, Name = Dal.Product.GetById((int)item?.PrudoctID!).Name, Price = (double)item?.Price!, ProductID = (int)item?.PrudoctID!, TotalPrice = (int)(item?.Amount * item?.Price)! };//transfers the DO list to BO
    ...
    public BO.Order UpdateShip(int orderId, DateTime? dat)
    {
        if (dat == null)
        {
            dat = DateTime.Now;
        ...
        try
        {
            ...
            if ((DOorder.ID >= 0) && DOorder.ID.ToString().Length == 4) // IF ORDER ID IS VALID
            {
                if (DOorder.ShipDate == null) // MANAGER CAN UPDATE SHIP DATE JUST IF THE ORDER IS NOT SHIPED YET
                {
                    ...
                    DOorder.ShipDate = dat; // UPDATE SHIP DATE
